...
#     location = db.Column(db.String(255), nullable=False)  # New field
#     cost = db.Column(db.Float, nullable=False)  # New field
#     accounts = db.relationship("Account", backref="resource", lazy=True)
#
#
# # Define the Account model (linking User and Resource)
...
#
#     @post_load
#     def add_accounts(self, data, **kwargs):
#         if "accounts" not in self.fields:
#             self.fields["accounts"] = fields.Nested(
#                 "AccountSchema", many=True, exclude=("user",)
#             )
#         return data
#
#
...
#         if "accounts" not in self.fields:
#             self.fields["accounts"] = fields.Nested(
#                 "AccountSchema", many=True, exclude=("resource",)
#             )
#         return data
#
#
# # Now define the Account schema
# class AccountSchema(ma.SQLAlchemyAutoSchema):
#     class Meta:
#         model = Account
#         load_instance = True
#
#     user = fields.Nested(UserSchema(only=("id", "username", "role")))
#     resource = fields.Nested(ResourceSchema(only=("id", "name", "location", "cost")))
#
#
# # Define the remaining schema instances
# user_schema = UserSchema()
# resource_schema = ResourceSchema()
# resources_schema = ResourceSchema(many=True)
...
#     db.session.commit()
#
#     # Create an account linking the user and the resource
#     identity = get_jwt_identity()
#     new_account = Account(user_id=identity["id"], resource_id=new_resource.id)
...
#
#
# # List all accounts (to see which user created which resource)
